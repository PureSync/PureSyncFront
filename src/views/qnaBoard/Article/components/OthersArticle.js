...
const ArticleItem = ({ data, isLastChild }) => {
    ...
    return (
        ...
        >
            ...
            <p className="mb-1">
                <TextEllipsis text={data.cmtContents} maxTextCount={40} />
            </p>
...
const OthersArticle = ({ articleId }) => {
    ...
    const { popularArticle, relatedArticle } = useSelector(
        (state) => state.knowledgeBaseQnaArticle.data.othersQnaArticle
    )
    const loading = useSelector(
        (state) => state.knowledgeBaseQnaArticle.data.otherLoading
    )
